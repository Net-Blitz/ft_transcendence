...
const GameInvitation = ({socketQueue}:any) => {
	const [gameInvitation, updateGameInvitation] = useState([] as {login: string, avatar: string}[]);
	...
	useEffect(() => {
		if (socketQueue && socketQueue.connected !== undefined)
		{
			...
			socketQueue.on("InviteGroupReceive", (data:any) => {
				...
				if (!inc)
					updateGameInvitation([...gameInvitation, {login: data.login, avatar: data.avatar}]);
		...
		<div className="game-invitation-main-div">
			{
				gameInvitation.map((invitation:any, index:number) => {
					return (
						<InvitationCard key={invitation.login} login={invitation.login} avatar={invitation.avatar} gameInvitation={gameInvitation} updateGameInvitation={updateGameInvitation} socketQueue={socketQueue} />
					)
				})
			}
		</div>
